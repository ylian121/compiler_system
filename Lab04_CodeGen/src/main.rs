...
impl Par {
    ...
    fn bin_op (&mut self, op:Tok, lhs:Vec<u8>, rhs:Vec<u8>) -> Option<Vec<u8>> {
        ...
        let ope;
        match op {
            Tok::Multiply => {ope = Vec::from(b"%mult");},
            Tok::Divide => {ope = Vec::from(b"%div");},
            Tok::Modulus => {ope = Vec::from(b"%mod");},
            Tok::Plus => {ope = Vec::from(b"%add");},
            Tok::Substract => {ope = Vec::from(b"%sub");},
            Tok::Less => {ope = Vec::from(b"%lt");},
            Tok::Greater => {ope = Vec::from(b"%gt");},
            Tok::LessEqual => {ope = Vec::from(b"%le");},
            Tok::GreaterEqual => {ope = Vec::from(b"%ge");},
            Tok::Equality => {ope = Vec::from(b"%eq");},
            Tok::NotEqual => {ope = Vec::from(b"%neq");},
    ...
    fn func(&mut self) -> Option<()> {
        ...
        // CodeGen1 - function header name done
        print!("%func {}(", String::from_utf8_lossy(&name)); // output function header
        ...
        loop {
            ...
            if let Tok::RightParen = self.tokens(1)[0] { // what if ')'?
                self.consume(1);
                // CodeGen1 - function line done
                print!(")\n"); // finished reading params, newline
                break;
            }
            if !first{
                self.expect(Tok::Comma)?; // remember '?'
                // CodeGen1 - function parameter comma done
                print!(", ");
            ...
            if let Tok::Ident(ref mut id) = self.tokens(1)[0] { // what if 'int'?
                ...
                // CodeGen1 - function arguments done
    ...
    fn stmts(&mut self) -> Option<()> {
        match self.tokens(1) {
            &mut [Tok::LeftCurly] => {
                self.consume(1);
                // CodeGen1 - code block opening done (unused)
                // println!("{{");
            },
        ...
        loop {
            if let Tok::RightCurly = self.tokens(1)[0] {
                self.consume(1);
                // CodeGen1 - code block ending done (unused)
                // println!("}}\n");
                break Some(());
            }
